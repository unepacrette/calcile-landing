...
## Current initiative: full interface redesign

Amaury is not satisfied with the current interface at all and wants it rebuilt properly --
scope is the entire app (marketing pages, /solve's result presentation, authenticated
pages), not one page in isolation. Two things drive this effort, both mandatory:

1. market-analyst's competitive research (see agents table below), delivered as
   COMPETITIVE_ANALYSIS.md at the repo root -- what competitors (WolframAlpha, Symbolab,
   Photomath, Mathway, Desmos) do well on the calculation-page side, and what drives
   conversion on strong SaaS landing pages, synthesized into a concrete direction for
   Calcile.
2. The apple-design skill (.claude/skills/apple-design/SKILL.md), installed for the
   interaction/motion/typography layer.

**Sequence, non-negotiable**: market-analyst researches and produces
COMPETITIVE_ANALYSIS.md first. That document gets reviewed (by Amaury/the PM) before
frontend-ui implements anything visual from it. Implementation then proceeds in
verified batches (see frontend-ui.md's "Sequencing" section) -- never as one large,
unreviewed diff touching every page at once. The FR/EN i18n structural-parity rule applies
to every batch without exception.
